...
class CartItemAPIView(ListCreateAPIView):
    ...
    def create(self,request,*args,**kwargs):
        ...
        cart.total = total 
        cart.save()
        push_notification(cart.user,"New cart product","you added a product to your cart"+product.title,)
...
class CartItemView(RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self,request,*args,**kwargs):
        ...
        if cart_item.cart.user != request.user:
            raise PermissionDenied("Sorry this cart not belong to you")
        cart_item.delete() 
        push_notification(cart_item.cart.user,
        "deleted cart product","you have been deleted this product:"+cart_item.product.title+"from your cart",)
        return Response({"detail":_("your item has been deleted.")},status=status.HTTP_204_NO_CONTENT,)
